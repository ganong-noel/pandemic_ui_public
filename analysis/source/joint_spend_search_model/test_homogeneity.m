...
FPUC_expiration_holder(1:5)=FPUC_expiration*[.25:.125:.75];
...
for iy = 1:5
    ...
    h = 0.7 * y;
    b = repshare * y;
    FPUC_onset=y*FPUC_expiration_holder(ii);

    search_rescale=(y^(1-mu))^.75;
    %search_rescale=1;
    ...
    for surprise = 0:1
        ...
        for beta_loop = 1:1
            ...
            for ib = 1:n_b
                ...
                v_u_guess(:, ib) = ((c_pol_u_guess(:, ib)).^(1 - mu) - 1) / (1 - mu) - search_rescale*(k_prepandemic * 0^(1 + gamma_prepandemic)) / (1 + gamma_prepandemic) + c_param_prepandemic*search_rescale;
            end
            ...
            for i_ben_profile = 1:n_ben_profiles_allowed
                ...
                for ib = 1:n_b
                    ...
                    v_u_pandemic_guess(:, ib, i_ben_profile) = ((c_pol_u_pandemic_guess(:, ib, i_ben_profile)).^(1 - mu) - 1) / (1 - mu) - search_rescale*(k * 0^(1 + gamma)) / (1 + gamma) + c_param*search_rescale;
                end
            ...
            while ((ave_change_in_C_percent > tol_c_percent) || (ave_change_in_S > tol_s)) && iter < maxiter
                ...
                for ib = 1:n_b
                    ...
                    tmp = min(1-recall_probs_regular(ib), max(0, (beta * (v_e_guess(:) - v_u_guess(:, min(ib + 1, n_b))) / (search_rescale*k_prepandemic)).^(1 / gamma_prepandemic)));
                   tmp(imag(tmp) ~= 0) = 0;
                ...
                for ib = 1:n_b
                    ...
                    v_u(:, ib) = ((c_pol_u(:, ib)).^(1 - mu) - 1) / (1 - mu) - search_rescale*(k_prepandemic * optimal_search(:, ib).^(1 + gamma_prepandemic)) / (1 + gamma_prepandemic) + c_param_prepandemic*search_rescale + beta * ((optimal_search(:, ib) + recall_probs_regular(ib)) .* interp1(Aprime, v_e_guess(:), a_prime_holder_u(:, ib), 'linear', 'extrap') + (1 - optimal_search(:, ib) - recall_probs_regular(ib)) .* interp1(Aprime, v_u_guess(:, min(ib + 1, n_b)), a_prime_holder_u(:, ib), 'linear', 'extrap'));
                end
                ...
                for i_ben_profile = 1:n_ben_profiles_allowed
                    ...
                    for ib = 1:n_b
                        ...
                        tmp = min(1-recall_probs_pandemic(ib), max(0, (beta * (v_e_guess(:) - v_u_pandemic_guess(:, min(ib + 1, n_b), i_ben_profile)) / (search_rescale*k)).^(1 / gamma)));
                        tmp(imag(tmp) ~= 0) = 0;
                    ...
                    for ib = 1:n_b
                        ...
                        v_u_pandemic(:, ib, i_ben_profile) = ((c_pol_u_pandemic(:, ib, i_ben_profile)).^(1 - mu) - 1) / (1 - mu) - search_rescale*(k * optimal_search_pandemic(:, ib, i_ben_profile).^(1 + gamma)) / (1 + gamma) + c_param*search_rescale +beta * ((recall_probs_pandemic(ib) + optimal_search_pandemic(:, ib, i_ben_profile)) .* interp1(Aprime, v_e_guess(:), a_prime_holder_u_pandemic(:, ib, i_ben_profile), 'linear', 'extrap') + (1 - optimal_search_pandemic(:, ib, i_ben_profile) - recall_probs_pandemic(ib)) .* interp1(Aprime, v_u_pandemic_guess(:, min(ib + 1, n_b), i_ben_profile), a_prime_holder_u_pandemic(:, ib, i_ben_profile), 'linear', 'extrap'));
                    end
        ...
        for t = 1:15
            ...
            for i = 1:num_unemployed_hh
                ...
                search_sim_regular(i, t) = min(1 - recall_probs_regular(ib), max(0, (beta * (diff_v) / (search_rescale*k_prepandemic)).^(1 / gamma_prepandemic)));
                ...
                search_sim_pandemic_noFPUC(i, t) = min(1 - recall_probs_pandemic(ib), max(0, (beta * (diff_v) / (search_rescale*k)).^(1 / gamma)));
                ...
                search_sim_pandemic_expect_onlyasseteffect(i, t) = min(1 - recall_probs_pandemic(ib), max(0, (beta * (diff_v) / (search_rescale*k)).^(1 / gamma)));
                ...
                search_sim_pandemic_expect_jan_start(i, t) = min(1 - recall_probs_pandemic(ib), max(0, (beta * (diff_v) / (search_rescale*k)).^(1 / gamma)));
                ...
                search_sim_pandemic_expect(i, t) = min(1 - recall_probs_pandemic(ib), max(0, (beta * (diff_v) / (search_rescale*k)).^(1 / gamma)));
                ...
                search_sim_pandemic_surprise(i, t) = min(1 - recall_probs_pandemic(ib), max(0, (beta * (diff_v) / (search_rescale*k)).^(1 / gamma)));
...
%writetable(table_agg_effects,fullfile(release_path,'table_agg_effects.csv'),'WriteRowNames',true);
...
table_distortions_bygroup=table();
table_distortions_bygroup.Bottom_Wage_Quintile_Workers('Percent Change In Aggregate Employment During 600 Supplement Period')=employment_distortion_lowwage_expiration;
table_distortions_bygroup.Bottom_Wage_Quintile_Workers('Percent Change In Aggregate Employment During 300 Supplement Period')=employment_distortion_lowwage_onset;
table_distortions_bygroup.Top_Wage_Quintile_Workers('Percent Change In Aggregate Employment During 600 Supplement Period')=employment_distortion_highwage_expiration;
table_distortions_bygroup.Top_Wage_Quintile_Workers('Percent Change In Aggregate Employment During 300 Supplement Period')=employment_distortion_highwage_onset
%writetable(table_distortions_bygroup,fullfile(release_path,'table_distortions_bygroup.csv'),'WriteRowNames',true);
...
%writetable(table_supplement_effects, fullfile(release_path,'table_supplement_effects.csv'), 'WriteRowNames', true);
...
%figure
%plot(1:14, mean_search_sim_pandemic_expect_bywage(:,1:14), 'LineWidth', 2)

change_job_finding(ii,:)=mean_search_sim_pandemic_expect_bywage(:,2)-mean_search_sim_pandemic_expect_bywage(:,3)
end

scatter(-FPUC_expiration_holder/repshare*100,change_job_finding)
xlabel('Percent Change in Benefits')
ylabel('Change in Exit Rate by Group')
legend('Wage Quintile 1','Wage Quintile 2','Wage Quintile 3','Wage Quintile 4','Wage Quintile 5')

data_y=change_job_finding;
data_x=repmat((FPUC_expiration_holder/repshare)'*100,5,1);
for i=1:5
    data_x_quintile(1:5,i)=i;
end
data_y2=reshape(data_y,25,1);
data_x2=reshape(data_x,25,1);
data_x_quintile2=reshape(data_x_quintile,25,1);
data=[data_y2 data_x2 data_x_quintile2];
data_subset=[data(21,:); data(17,:); data(13,:); data(9,:); data(5,:)];
data_subset(1,2)=41.667;
data_subset(2,2)=62.5;
data_subset(4,2)=104.1667;
data_subset(5,2)=125;
%slope of the subset data (empirical specification) is 5% not 5pp larger
%than slope of full data ("truth")
