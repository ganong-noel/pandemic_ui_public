display('Simulating Full Model w/ heterogeneity in liquidity')
close all
tic
...
%data_high_liq = readtable('df_monthly_collapsed_high_liquidity_for_plot.csv');
idx_emp = (string(data_high_liq.category) == 'Spending (total)') & (string(data_high_liq.group) == 'Employed') & (string(data_high_liq.measure) == 'mean') & data_high_liq.periodid >= 201901;
idx_u = (string(data_high_liq.category) == 'Spending (total)') & startsWith(string(data_high_liq.group), 'Unemployed') & (string(data_high_liq.measure) == 'mean') & data_high_liq.periodid >= 201901;
data_high_liq_e = data_high_liq(idx_emp, :);
data_high_liq_u = data_high_liq(idx_u, :);
total_spend_e = data_high_liq_e.value;
total_spend_u = data_high_liq_u.value;
total_spend_e = total_spend_e(13:end);
total_spend_u = total_spend_u(13:end);
us_v1_highliq=total_spend_u./total_spend_e-1;
us_v1_highliq=us_v1_highliq-mean(us_v1_highliq(1:1));
spend_data_high_liq=us_v1_highliq*100;

idx_emp = (string(data_high_liq.category) == 'Income') & (string(data_high_liq.group) == 'Employed') & (string(data_high_liq.measure) == 'mean') & data_high_liq.periodid >= 201901;
idx_u = (string(data_high_liq.category) == 'Income') & startsWith(string(data_high_liq.group), 'Unemployed') & (string(data_high_liq.measure) == 'mean') & data_high_liq.periodid >= 201901;
data_high_liq_e = data_high_liq(idx_emp, :);
data_high_liq_u = data_high_liq(idx_u, :);
income_e = data_high_liq_e.value;
income_u = data_high_liq_u.value;
income_e = income_e(13:end);
income_u = income_u(13:end);

idx_emp = (string(data_high_liq.category) == 'Checking account balance') & (string(data_high_liq.group) == 'Employed') & (string(data_high_liq.measure) == 'mean') & data_high_liq.periodid >= 201901;
idx_u = (string(data_high_liq.category) == 'Checking account balance') & startsWith(string(data_high_liq.group), 'Unemployed') & (string(data_high_liq.measure) == 'mean') & data_high_liq.periodid >= 201901;
data_high_liq_e = data_high_liq(idx_emp, :);
data_high_liq_u = data_high_liq(idx_u, :);
checking_e_high_liq = data_high_liq_e.value;
checking_u_high_liq = data_high_liq_u.value;
%checking_e = income_e(13:end);
%checking_u = income_u(13:end);

mpc_expiration_high_liq=((total_spend_u(8)-total_spend_u(7))-(total_spend_e(8)-total_spend_e(7)))/((income_u(8)-income_u(7))-(income_e(8)-income_e(7)));

spend_ratio_high_liq=(total_spend_e(1)/income_e(1));

%data_low_liq = readtable('df_monthly_collapsed_low_liquidity_for_plot.csv');
idx_emp = (string(data_low_liq.category) == 'Spending (total)') & (string(data_low_liq.group) == 'Employed') & (string(data_low_liq.measure) == 'mean') & data_low_liq.periodid >= 201901;
idx_u = (string(data_low_liq.category) == 'Spending (total)') & startsWith(string(data_low_liq.group), 'Unemployed') & (string(data_low_liq.measure) == 'mean') & data_low_liq.periodid >= 201901;
data_low_liq_e = data_low_liq(idx_emp, :);
data_low_liq_u = data_low_liq(idx_u, :);
total_spend_e = data_low_liq_e.value;
total_spend_u = data_low_liq_u.value;
total_spend_e = total_spend_e(13:end);
total_spend_u = total_spend_u(13:end);
us_v1_lowliq=total_spend_u./total_spend_e-1;
us_v1_lowliq=us_v1_lowliq-mean(us_v1_lowliq(1));
spend_data_low_liq=us_v1_lowliq*100;

idx_emp = (string(data_low_liq.category) == 'Income') & (string(data_low_liq.group) == 'Employed') & (string(data_low_liq.measure) == 'mean') & data_low_liq.periodid >= 201901;
idx_u = (string(data_low_liq.category) == 'Income') & startsWith(string(data_low_liq.group), 'Unemployed') & (string(data_low_liq.measure) == 'mean') & data_low_liq.periodid >= 201901;
data_low_liq_e = data_low_liq(idx_emp, :);
data_low_liq_u = data_low_liq(idx_u, :);
income_e = data_low_liq_e.value;
income_u = data_low_liq_u.value;
income_e = income_e(13:end);
income_u = income_u(13:end);

idx_emp = (string(data_low_liq.category) == 'Checking account balance') & (string(data_low_liq.group) == 'Employed') & (string(data_low_liq.measure) == 'mean') & data_low_liq.periodid >= 201901;
idx_u = (string(data_low_liq.category) == 'Checking account balance') & startsWith(string(data_low_liq.group), 'Unemployed') & (string(data_low_liq.measure) == 'mean') & data_low_liq.periodid >= 201901;
data_low_liq_e = data_low_liq(idx_emp, :);
data_low_liq_u = data_low_liq(idx_u, :);
checking_e_low_liq = data_low_liq_e.value;
checking_u_low_liq = data_low_liq_u.value;

subplot(1,2,1)
plot(1:36,checking_e_high_liq,1:36,checking_u_high_liq)
subplot(1,2,2)
plot(1:36,checking_e_low_liq,1:36,checking_u_low_liq)

figure
subplot(1,2,1)
plot(1:36,checking_e_high_liq/checking_e_high_liq(1),1:36,checking_u_high_liq/checking_u_high_liq(13))
subplot(1,2,2)
plot(1:36,checking_e_low_liq/checking_e_low_liq(1),1:36,checking_u_low_liq/checking_e_low_liq(13))



mpc_expiration_low_liq=((total_spend_u(8)-total_spend_u(7))-(total_spend_e(8)-total_spend_e(7)))/((income_u(8)-income_u(7))-(income_e(8)-income_e(7)))

spend_ratio_low_liq=(total_spend_e(1)/income_e(1));
...
for beta_het_loop=1:4

    if beta_het_loop==1
        beta_normal = beta_targetwaiting-.01;
    elseif beta_het_loop==2
        beta_normal = beta_target500MPC+.0045;
...
if beta_het_loop==1
    mean_c_sim_pandemic_surprise_dollars_impatient=mean_c_sim_pandemic_surprise_dollars;
    mean_c_sim_pandemic_surprise_impatient=mean_c_sim_pandemic_surprise;
    mean_a_sim_pandemic_surprise_impatient=mean_a_sim_pandemic_surprise;
    mean_c_sim_e_impatient=mean_c_sim_e;
    mean_search_sim_pandemic_surprise_impatient=mean_search_sim_pandemic_surprise;
elseif beta_het_loop==2
    mean_c_sim_pandemic_surprise_dollars_patient=mean_c_sim_pandemic_surprise_dollars;
    mean_c_sim_pandemic_surprise_patient=mean_c_sim_pandemic_surprise;
    mean_a_sim_pandemic_surprise_patient=mean_a_sim_pandemic_surprise;
    mean_c_sim_e_patient=mean_c_sim_e;
    mean_search_sim_pandemic_surprise_patient=mean_search_sim_pandemic_surprise;
elseif beta_het_loop==3
    mean_c_sim_pandemic_surprise_dollars_low_a=mean_c_sim_pandemic_surprise_dollars;
    mean_c_sim_pandemic_surprise_low_a=mean_c_sim_pandemic_surprise;
    mean_a_sim_pandemic_surprise_low_a=mean_a_sim_pandemic_surprise;
    mean_c_sim_e_low_a=mean_c_sim_e;
    mean_search_sim_pandemic_surprise_low_a=mean_search_sim_pandemic_surprise;
elseif beta_het_loop==4
    mean_c_sim_pandemic_surprise_dollars_high_a=mean_c_sim_pandemic_surprise_dollars;
    mean_c_sim_pandemic_surprise_high_a=mean_c_sim_pandemic_surprise;
    mean_a_sim_pandemic_surprise_high_a=mean_a_sim_pandemic_surprise;
    mean_c_sim_e_high_a=mean_c_sim_e;
    mean_search_sim_pandemic_surprise_high_a=mean_search_sim_pandemic_surprise;
end
...
mpc_overall=.08*mpc_impatient.surprise('500 quarterly')+.92*mpc_patient.surprise('500 quarterly');

mean_c_sim_e_average=.08*mean_c_sim_e_impatient+.92*mean_c_sim_e_patient;



mean_c_sim_patient=mean_c_sim_pandemic_surprise_patient./mean_c_sim_e_average(1:numsim);
mean_c_sim_impatient=mean_c_sim_pandemic_surprise_impatient./mean_c_sim_e_average(1:numsim);
...
hold on
%plot(1:11, spend_data_high_liq(1:11), '--o', 'Color', qual_blue, 'MarkerFaceColor', qual_blue, 'LineWidth', 2)
plot(1:11, mean_c_sim_pandemic_surprise_dollars_high_a(1:11)/total_spend_u(1)*100, '-s', 'Color', qual_purple, 'MarkerFaceColor', qual_purple, 'LineWidth', 2)
%plot(1:11, spend_data_low_liq(1:11), '--o', 'Color', qual_orange, 'MarkerFaceColor', qual_orange, 'LineWidth', 2)
plot(1:11, mean_c_sim_pandemic_surprise_dollars_low_a(1:11)/total_spend_u(1)*100, '-s', 'Color', qual_yellow, 'MarkerFaceColor', qual_yellow, 'LineWidth', 2)
%plot(1:11, mean_c_sim_pandemic_surprise_noFPUC_dollars(1:11)/total_spend_u(1)*100, '-o', 'Color', [0.6 0.6 0.6], 'MarkerFaceColor', [0.6 0.6 0.6], 'LineWidth', 2)
%legend('Low 2019 assets - data', 'High 2019 assets - data','Low 2019 assets - model', 'High 2019 assets - model', 'Location', 'SouthWest', 'FontSize', 14)
set(gca, 'fontsize', 12);
title({'Model: No Perm Heterogeneity'},'fontsize', 14,'fontweight','normal')
%ylabel('\Delta $')
yticks([-20 -10 0 10 20])
yticklabels({'-20%','-10%', '0%','10%', '20%'})
ylabel('Spending (U vs. E % change from Jan 20)');
xlim([1 11])
xticks([1  3  5  7  9  11])
ylim([-10 20])
xticklabels({'Jan 20','Mar 20','May 20','Jul 20','Sep 20', 'Nov 20'})
set(gca, 'Layer', 'top');
% set(gcf, 'PaperPosition', [0 0 10.4 4.5]); %Position the plot further to the left and down. Extend the plot to fill entire paper.
set(gcf, 'PaperPosition', [0 0 10 5]); %Position the plot further to the left and down. Extend the plot to fill entire paper.
set(gcf, 'PaperSize', [10 5]); %Keep the same paper size
...
hold on
%plot(1:11, spend_data_high_liq(1:11), '--o', 'Color', qual_blue, 'MarkerFaceColor', qual_blue, 'LineWidth', 2)
plot(1:11, mean_c_sim_pandemic_surprise_dollars_patient(1:11)/total_spend_u(1)*100, '-s', 'Color', qual_purple, 'MarkerFaceColor', qual_purple, 'LineWidth', 2)
%plot(1:11, spend_data_low_liq(1:11), '--o', 'Color', qual_orange, 'MarkerFaceColor', qual_orange, 'LineWidth', 2)
plot(1:11, mean_c_sim_pandemic_surprise_dollars_impatient(1:11)/total_spend_u(1)*100, '-s', 'Color', qual_yellow, 'MarkerFaceColor', qual_yellow, 'LineWidth', 2)
%plot(1:11, mean_c_sim_pandemic_surprise_noFPUC_dollars(1:11)/total_spend_u(1)*100, '-o', 'Color', [0.6 0.6 0.6], 'MarkerFaceColor', [0.6 0.6 0.6], 'LineWidth', 2)
set(gca,'fontsize', 12);
title({'Model: Perm Heterogeneity'},'fontsize', 14,'fontweight','normal')
%legend('Low 2019 assets - data', 'High 2019 assets - data','Low 2019 assets - model', 'High 2019 assets - model', 'Location', 'SouthWest', 'FontSize', 14)
%ylabel('\Delta $')
yticks([-20 -10 0 10 20])
yticklabels({'-20%','-10%', '0%','10%', '20%'})
...
%set(gcf, 'PaperSize', [4.5 2]); %Keep the same paper size
%pbaspect([4.5 2 1])
lg  = legend('High 2018 Liquidity', 'Low 2018 Liquidity', 'FontSize', 14);
lg.Layout.Tile = 'South';
nexttile
%subplot(1,2,1)
p = patch([4 4 7 7], [-20 30 30 -20], [0.9 0.9 0.9], 'EdgeColor', 'none');
set(get(get(p(1), 'Annotation'), 'LegendInformation'), 'IconDisplayStyle', 'off');
hold on
plot(1:11, spend_data_high_liq(1:11), '-s', 'Color', qual_purple, 'MarkerFaceColor', qual_purple, 'LineWidth', 2)
%plot(1:11, mean_c_sim_pandemic_surprise_dollars_high_a(1:11)/total_spend_u(1)*100, '-s', 'Color', qual_blue, 'MarkerFaceColor', qual_blue, 'LineWidth', 2)
plot(1:11, spend_data_low_liq(1:11), '-s', 'Color', qual_yellow, 'MarkerFaceColor', qual_yellow, 'LineWidth', 2)
%plot(1:11, mean_c_sim_pandemic_surprise_dollars_low_a(1:11)/total_spend_u(1)*100, '-s', 'Color', qual_orange, 'MarkerFaceColor', qual_orange, 'LineWidth', 2)
%plot(1:11, mean_c_sim_pandemic_surprise_noFPUC_dollars(1:11)/total_spend_u(1)*100, '-o', 'Color', [0.6 0.6 0.6], 'MarkerFaceColor', [0.6 0.6 0.6], 'LineWidth', 2)
%legend('Low 2019 assets - data', 'High 2019 assets - data','Low 2019 assets - model', 'High 2019 assets - model', 'Location', 'SouthWest', 'FontSize', 14)
set(gca, 'fontsize', 12);
title({'Data'},'fontsize', 14,'fontweight','normal')
%ylabel('\Delta $')
yticks([-20 -10 0 10 20])
yticklabels({'-20%','-10%', '0%','10%', '20%'})
xlim([1 11])
...
xticklabels({'Jan 20','Mar 20','May 20','Jul 20','Sep 20', 'Nov 20'})
set(gca, 'Layer', 'top');
set(gcf, 'Position',  [500,500,1100,460])
set(gcf, 'PaperPosition', [0 0 10 3]); %Position the plot further to the left and down. Extend the plot to fill entire paper.
set(gcf, 'PaperSize', [13 2]); %Keep the same paper size
%lg.FontSize = 14
set(gca, 'Layer', 'top');
fig_paper_12 = gcf;
...
mean_a_sim_pandemic_surprise_patient(1)
mean_a_sim_pandemic_surprise_impatient(1)

mean_a_sim_pandemic_surprise_high_a(1)
mean_a_sim_pandemic_surprise_low_a(1)
